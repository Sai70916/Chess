namespace Chess {
    public static class FenUtility {
        ...
        static Dictionary<char, int> pieceTypeFromSymbol = new Dictionary<char, int> () {
            ['k'] = Piece.King, ['p'] = Piece.Pawn, ['r'] = Piece.Rook, 
            ['n'] = Piece.Knight, ['b'] = Piece.Bishop, ['q'] = Piece.Queen
        };
        ...
        public static LoadedPositionInfo  PositionFromFen(string fen) {
            ...
            if (sections.Length > 3) {
                string enPassantFileName = sections[3][0].ToString (); // We dont need rank
                // Add check to make sure that the file in possible in a board after adding that in board representation
                    // Add epFile to loadedPositionInfo
            }
